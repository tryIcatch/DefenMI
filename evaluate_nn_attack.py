...
name='noise_data_noise.npz'
evaluation_noise_filepath=result_folder+"/attack/"+"{}".format(name)
...
shape=npz_defense['defense_output'].shape

f_evaluate_noise=npz_defense['defense_output'].reshape(shape[0],shape[-1])
f_evaluate_origin=npz_defense['tc_output'].reshape(shape[0],shape[-1])


f_evaluate_defense=np.zeros(f_evaluate_noise.shape,dtype=float)
np.random.seed(100)
...
print(np.array_equal(f_evaluate_defense,f_evaluate_noise))
...
model.compile(loss=tf.keras.losses.categorical_crossentropy,optimizer=tf.keras.optimizers.SGD(lr=0.001),metrics=['accuracy'])
...
############define attack_v1 model#####################
input_shape=b_train.shape[1:]
model=fccnet.model_attack_nn_4layer(input_shape=input_shape,labels_dim=num_classes)
model.compile(loss=tf.keras.losses.binary_crossentropy,optimizer=tf.keras.optimizers.SGD(lr=0.01),metrics=['accuracy'])
model.summary()
...
f_evaluate_noise=npz_defense['defense_output'].reshape(shape[0],shape[-1])
f_evaluate_origin=npz_defense['tc_output'].reshape(shape[0],shape[-1])
f_evaluate_origin_score=npz_defense['predict_origin'].reshape(shape[0],1)
f_evaluate_defense_score=npz_defense['predict_modified'].reshape(shape[0],1)
...
for epsilon_value in epsilon_value_list:
    ...
    total_confidence_loss = 0.0  # 初始化总置信度损失
...
print("average_confidence loss list: {}".format(epsilon_value_list))
