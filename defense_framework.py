...
for cls, noisy_samples in classified_samples.items():
    ...
    noisy_sample = np.random.laplace(
        loc=0,
        scale=delta / epsilon,
        size=(noisy_samples.shape[0], noisy_samples.shape[1])
    )
    x_evaluate_n[class_indices] = x_evaluate[class_indices] + noisy_sample
class_logits = {}
class_confidence = {}
...
np.savez(result_folder + "/attack/" + "noise_data_noise.npz",
